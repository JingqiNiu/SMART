...
def _get_circle_by_center_bbox(shape, center, bbox, radius):
    ...
    center_mask = cv2.circle(
        center_mask, center_tmp[::-1], int(radius), (1), -1)
...
def black_cut_fun(img_rgb):
    ...
    marker_set = (
        (5, 5), (5, img_shape[1] - 5), (img_shape[0] - 5, 5), (img_shape[0] - 5, img_shape[1] - 5))
    marker_color_set = []
    ...
    if marker_color_set_std >= marker_color_std_tol:
        raise Exception(
            "CHANGE BORDER COLOR MODULE: failed to get consistent border color based on four markers.")
    ...
    circle_mask = _get_circle_by_center_bbox(
        img_shape, (h//2, w//2), bbox, radius)
